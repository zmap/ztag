            ...
            if annotation.tests:
                ...
                for k, v in annotation.tests.iteritems():
                    ...
                    if set(output.tags) != set(expected_tags):
                        errors.append("  - %s (%s): %s != %s" % (k, "tags", str(output.tags), str(expected_tags)))
                        tags_match = False
                    if output.local_metadata.to_dict(with_description=False) != expected_local_metadata:
                        errors.append("  - %s (%s): %s != %s" % (k, "local metadata", str(output.local_metadata.to_dict()), str(expected_local_metadata)))
                        local_metadata_match = False
                    if output.global_metadata.to_dict(with_description=False) != expected_global_metadata:
                        errors.append("  - %s (%s): %s != %s" % (k, "global metadata", str(output.global_metadata.to_dict()), str(expected_global_metadata)))
